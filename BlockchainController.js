...
class BlockchainController {
    ...
    requestOwnership() {
        this.app.post("/requestValidation", async (req, res) => {
            //console.log(`/requestValidation body = ${JSON.stringify(req.body)}`);
            if (req.body.address) {
                ...
                return res.status(500).send("Check the damn Body Parameter!");
            }
        });
    }
    ...
    submitStar() {
        this.app.post("/submitstar", async (req, res) => {
            if (req.body.star && req.body.address && req.body.message && req.body.signature) {
                const address = req.body.address;
    ...
    getValidateChain() {
        this.app.get("/validateChain", async (req, res) => {
            ...
            await this.blockchain.validateChain().then(
                (result) => {
                    return res.status(200).send("Chain is Validated.");
                },
                (error) => {
                    return res.status(404).send("Chain is has been compromised.");
                }
            );
    ...
    getAllBlocks() {
        this.app.get("/blocks", async (req, res) => {
                ...
                try {
                    let allBlocks = this.blockchain.getAllBlocks();
                    if (allBlocks) {
